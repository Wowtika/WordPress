(($) => {
	...
	class FilterMini extends FilterCore {
		...
		clearSelects(type){
			...
			case 'region':
			_this.$year.html('<option value="" data-loading selected>Year</option>');
			_this.$year.attr('disabled', true);
			_this.rebuildSelect( _this.$year );

			case 'year':
			_this.$make.html('<option value="" data-loading selected>Make</option>');
			_this.$make.attr('disabled', true);
			_this.rebuildSelect( _this.$make );

			case 'make':
			_this.$model.html('<option value="" data-loading selected>Model</option>');
	...
	class FilterFull extends FilterCore {
				...
				success: function(res){
					if( res && res.success ) {
						if( res.data ) {
							...
							} else if ( res.data.part_applications ) {
								console.log('нужно уточнить параметры');
								load_catalog.html('');
								$('#catalog_row').html('done');
							} else{
			...
			return $.ajax({
				...
				success: function(res){
					if( res && res.success ) {
						if( res.data ) {
							console.log(res);
							catalog_auto_title.html(make + ' ' + model + ' ' + submodel + ' ' + year);
			...
			$.each(data, function(i, parts) {
				if (parts !== null && typeof parts == 'object') {
					if (parts.part_id) {
						...
						part.classList.add("item-catalog");
						part.classList.add("catalog__item");
						part.setAttribute('data-part_id', item.part_id);

						let part_icon = document.createElement('div');
						part_icon.classList.add("item-catalog__header-icons");
						part_icon.innerHTML = '<a href="#"><img src="/wp-content/themes/friction-master/assets/img/catalog/catalog-card1.svg" alt="" /></a><a href="#"><img src="/wp-content/themes/friction-master/assets/img/catalog/catalog-card2.svg" alt="" /></a><a href="#"><img src="/wp-content/themes/friction-master/assets/img/catalog/catalog-card3.svg" alt="" /></a>';

						let part_heading = document.createElement('div');
						part_heading.classList.add("item-catalog__header-heading");
						part_heading.innerHTML = item.part_number;
						...
						part_footer.innerHTML = '<a href="/product?part_id='+item.part_id+'">Show more</a><button type="submit" class="item-catalog__footer-button buy-button">BUY</button>';

						part.append(part_header);
						part.append(part_img);
						part.append(part_footer);

						catalog_row.append( part );
					} else {
						_this.create_parts(parts);
					}
				}
			});
		}
		...
		loadingBlock($block, load = true){
			$block = $($block);
			let loading = '<div class="loading_block"><div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> <div class="loading_wave"></div> </div>';
	...
	modules_flsModules.filter_mini = new FilterMini('[data-filter="mini"]', modules_flsModules.select);
	modules_flsModules.filter_top = new FilterTop('[data-filter="top"]', modules_flsModules.select);
	modules_flsModules.filter_full = new FilterFull('[data-filter="full"]', modules_flsModules.select);
...
***************************************************************************** */
	...
	if (document.querySelector(".hideMe")) {
		...
		function slideAnim(e) {
			...
			if (this.id === "dragger") activeSlide = offsets.indexOf(this.endX); else {
				if (gsap.isTweening(container)) return;
				if (this.id === "leftArrow" || this.id === "rightArrow") activeSlide = this.id === "rightArrow" ? activeSlide += 1 : activeSlide -= 1; else if (this.className === "dot") activeSlide = this.index; else activeSlide = e.deltaY > 0 ? activeSlide += 1 : activeSlide -= 1;
			}
			activeSlide = activeSlide < 0 ? 0 : activeSlide;
			activeSlide = activeSlide > slides.length - 1 ? slides.length - 1 : activeSlide;
			if (oldSlide === activeSlide) return;
			if (this.id != "dragger") gsap.to(container, dur, {
		...
		function sizeIt() {
			...
			for (let i = 0; i < slides.length; i++) offsets.push(-slides[i].offsetLeft);
			gsap.set(container, {
				x: offsets[activeSlide]
			});
		...
		if (ScrollTrigger.isTouch !== 1) if (document.querySelector(".page_home")) {
			gsap.fromTo(".prefs-lines__image", {
				y: 0,
				x: 300
			}, {
				x: -100,
			...
			gsap.fromTo(".prodlines-lines__image", {
				x: 0,
				y: 0,
				scale: 1,
			...
			ctaRedLinesTl.fromTo(".cta-red-lines", {
				x: "-50%",
				y: 0
			}, {
				x: "0%",
	...
	if (document.querySelector(".page_aboute")) {
        ...
        const getUseableWidth = () => timelineContainer.scrollWidth - window.innerWidth;
		...
		const getDraggableWidth = () => track.offsetWidth * .5 - lastItemWidth();
